...
class NegativeSentimentAnalyzer:
    ...
    def is_negative(self, text: str) -> bool:
        prompt = f'Consider the text delimited by triple backticks and intelligently determine if it contains a negative or hateful sentiment. The goal should be to not just identify isolated bad words but to understand the context of sentences in which they are used. The aim is to maintain a positive and respectful community environment without stifling genuine conversation. Ensure false positives (e.g., "Fuck yeah that was great") are minimized while true negatives (e.g., "Fuck, that was so bad") are captured. Keep in mind that the text might be in a financial context. So negative in terms of finance, like "we had a loss today" or "the price just went down", should not be marked as negative.\n```{text}```\nProvide a single character as output, either 1 or 0, where 1 means that the text has negative sentiment, and 0 means it does not. Do not explain your answer.'
